    ...
    try {
      ...
      if (rawSession) {
        ...
        if (sessionUpdate._addSubagent) {
          const agentId = sessionUpdate._addSubagent;
          if (!rawSession.subagents.includes(agentId)) {
            rawSession.subagents.push(agentId);
          }
          // Only record start details once — don't overwrite an existing entry
          if (!rawSession.subagentDetails[agentId]) {
            rawSession.subagentDetails[agentId] = {
        ...
        if (sessionUpdate._removeSubagent) {
          const agentId = sessionUpdate._removeSubagent;
          rawSession.subagents = rawSession.subagents.filter(a => a !== agentId);
          if (rawSession.subagentDetails[agentId]) {
            rawSession.subagentDetails[agentId].endedAt = Date.now();
          }
        }
