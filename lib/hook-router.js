...
function createHookRouter(store, broadcast, cancelRemoval) {
  return function handleHook(req, res) {
    try {
      ...
      // New event arrived — terminal is still alive, cancel any pending removal
      if (cancelRemoval) cancelRemoval(sessionId);
